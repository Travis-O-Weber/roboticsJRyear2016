...
// Scale joysticks values for Motors
...
task drive()
{
    while (true){
        ...
        arcadeDrive(scaleForMotor(joystick.joy1_x1),scaleForMotor(joystick.joy1_y1));
        wait1Msec(100);
    }
}
...
task main()
{
  ...
  while (true)
  {
  	...
  	if(abs(joystick.joy2_x2) > 10) {
  		motor[Flag]=joystick.joy2_x2;	// Turn motor to raise the flag
  	} else {
  		motor[Flag]=0;
  	}
  	...
  	if(abs(joystick.joy2_y1) > 5) {
  		motor[Arm]=scaleForMotor(joystick.joy2_y1) * .3; // Move the Arm using expo control
  	} else {
  		motor[Arm]=0;
  	}
  	// Move the Lift up
